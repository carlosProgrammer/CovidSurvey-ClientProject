	 ...
	 if ($xmlDoc->col[0]->dbconnected== '1'){
		...
		if ($xmlDoc->col[0]->source== 'upload'){
			...
			if (substr($xmlDoc->col[$col]->file,0,6)=="https:" ) {
				$connectionstatus='<div class="fa fa-cloud-download" style="color:#669933;"></div>';
			}
		}
	  ?>
...
if (($_POST['node']=="2") || ($_POST['node']=="n") || ($_POST['node']=='o') || ($_POST['node']=='g') || ($_POST['node']=='s')) { //DB button pressed
?>
<script>
...
if (isset($_POST['node']) && ($_POST['node']=="0")) { //Setting button pressed
// do nothing ?>
<SCRIPT>
   ...
   stop : function(event,ui) {
		...
		var col_size = document.getElementById("wraplayoutsettingchart").clientWidth;	
			...
			if (endW > col_size) { 
				...
				Plotly.relayout('collayoutsettings', {
					  height:endH-50,
					  width: endW - 55
				});
				...
				<?php 
				 if ($xmlDoc->col[$key]->dbconnected== '1'){ ?>
					...
					if ($xmlDoc->col[$key]->source== 'upload'){
						...
						if (substr($xmlDoc->col[$key]->file,0,6)=="https:" ) { ?>
...
<?php
		...
		<?php 
		if ((isset($xml->col[$i]->type)) && (!empty($result[$i])) ) {
		if (!(($xml->col[$i]->type=='map')  || ($xml->col[$i]->type=='number'))) {?>
